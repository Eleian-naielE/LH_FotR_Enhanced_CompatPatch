...
function RepublicHeroes:new(gc, herokilled_finished_event, human_player, hero_clones_p2_disabled, id)
	...
	general_data = {
		...
		full_list = { --All options for reference operations
			...
			["Rohn"] = {"ROHN_ASSIGN",{"ROHN_RETIRE"},{"ROHN_AT_TE"},"Rohn", ["Companies"] = {"ROHN_TEAM"}}, -- FotR_Enhanced
...
function RepublicHeroes:CommandStaff_Initialize(command_staffs)
	...
	if tech_level >= 4 then
		...
		self:Add_Fighter_Set("Jag_ARC170_Location_Set")
		self:Add_Fighter_Set("Hawk_Location_Set")
...
function RepublicHeroes:on_galactic_hero_killed(hero_name, owner, killer_name)
	...
	elseif tag_admiral == "Block" then
		if block_second_chance_used == false then
			block_second_chance_used = true
			if hero_name == "BLOCK_VIGILANCE" then
				return
			end
			admiral_data.full_list["Block"].unit_id = 2 --BLOCK_VIGILANCE
...
function RepublicHeroes:Phase_II()
	...
	for _, clone_name in ipairs({"Fordo", "Alpha", "Ordo", "Aden", "Prudii"}) do
...
function RepublicHeroes:Venator_Heroes()
	...
	if not self.Venator_init then
		...
		if self.starting_tech_level <=3 then -- FotR_Enhanced
...
function RepublicHeroes:Order_66_Handler()
	...
	Handle_Hero_Add_2("Rohn", general_data)
	...
	Handle_Hero_Exit_2("Prudii", commando_data)
	Autem_Checks = -1
...
function RepublicHeroes:Geen_Unlock()
	Handle_Hero_Add_2("Geen", general_data)
end
...
function Forral_Check() 
	...
	if Forral_Checks == 2 then
		...
		self:Remove_Fighter_Set("Bythen_Forral_Location_Set")
	end
